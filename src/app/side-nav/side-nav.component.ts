...
@Input ()logs
  constructor() {
    ...
    this.addDocmedium("Observing Ants (1/1)", "This section provides an overview of some of the important behaviors from ants, and how we attempted to incorporate that in our application.", "Ants walk in columns", "Ants often walk in columns. Even though our simulation is far too simple to be able to mimick the exact reasons why, we can reward the ants for walking in a column. Our solution: Traversing a cell another ant walked recently provides a pathfinding advantage, making the pathfinding cost analysis for the ant cheaper. This results in the ants having a  preference for walking behind other ants.", "Ants have different roles", "Ants fulfill different types of roles in real ant colonies. Although in an actual ant colony the role will vary based on a number of factors, we made it so that an ant is created as a specific type.");
